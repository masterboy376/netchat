...
const MoreOption = () => {
    ...
    return (
        <div className={`w-full transition-all duration-300 sm:hidden border-l ${isDark ? 'text-gray-300 border-gray-700 bg-gray-900 ' : 'text-gray-900 border-gray-200 bg-white'} ${isRightBar ? 'translate-x-0' : 'translate-x-full'} sm:hidden block fixed top-0 right-0 h-screen`}>
            <div className={`flex items-center justify-between border-b z-10 p-1 ${isDark ? 'border-gray-700' : 'border-gray-200'} shadow-sm`}>
            ...
            <button className={`mx-auto w-1/2 text-white bg-blue-600 hover:bg-blue-700 font-medium p-2 rounded-xl justify-evenly flex items-center`} onClick={logout}>
                Log out
            </button>
